...
function checkIfLayerHasRefresh(
  ...
  return (
    !!layer &&
    Object.prototype.hasOwnProperty.call(layer, "refresh") &&
...
function createRenderer() {
  ...
  const penetrationSymbol = new SimpleMarkerSymbol(
    SimpleMarkerSymbol.STYLE_SQUARE,
    10,
    lineSymbol,
    new Color("red")
...
function formatResults(graphic: Graphic) {
  const message = `A structure ${graphic.attributes.agl}' above ground level ${
    graphic.attributes.penetratesSurface ? " would " : " would not "
  ...
  > = {
    "Penetration of <abbr title='Federal Aviation Regulations'>FAR</abbr> Surface occurred at":
      formatFeetAsFeetAndInchesAndMeters(
        graphic.attributes.distanceFromSurface
      ),
    "Terrain Elevation": formatFeetAsFeetAndInchesAndMeters(
      graphic.attributes.terrainElevation
    ),
  };
  if (graphic.attributes.penetratesSurface) {
    data[
      ...
      graphic.attributes.penetrationOfSurface
    );
  }
...
export default class ArcGisUI extends UI {
  ...
  private updateMapMarker(dmsCoordinates?: Point | DmsCoordinates) {
    if (dmsCoordinates) {
      const point =
        dmsCoordinates instanceof Point
          ? dmsCoordinates
          : new Point({
      ...
      if (checkIfLayerHasRefresh(this._markerLayer))
  ...
  public set map(value: EsriMap | null) {
    ...
    if (this._map) {
      ...
      this._draw.on("draw-complete", (drawResponse) => {
        ...
        this.form.x.value = drawPoint.x.toString();
        this.form.y.value = drawPoint.y.toString();
        this.updateMapMarker(drawPoint);
      ...
      this.form.addEventListener("add-from-map", () => {
        (this._draw as Draw).activate(Draw.POINT);
      });
      ...
      this._resultLayer.on("graphic-add", (e) => {
        ...
        if (graphic) {
          ...
          if (this._map) {
            const infoWindow = this._map.infoWindow as Popup;
      ...
      this._map.addLayer(this._resultLayer);
    }
  }
}
